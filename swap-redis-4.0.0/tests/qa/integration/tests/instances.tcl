...
proc cleanup {} {
    ...
    # some case fail need to check the log file.
    if {$::failed == 0} {
        foreach dir $::dirs {
...
proc parse_options {} {
    for {set j 0} {$j < [llength $::argv]} {incr j} {
        ...
        if {$opt eq "--single"} {
            ...
            puts "--nodepairs             How many master and slave pairs."
...
proc run_tests {} {
    ...
    write_cluster_test_result_report "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"
    write_cluster_test_result_report "<testsuites>"
    foreach test $tests {
        if {$::run_matching ne {} && [string match $::run_matching $test] == 0} {
            continue
        }
        if {[file isdirectory $test]} continue
        set ::curscript [lindex [split [lindex [file split $test] end] .] 0]
...
#Generate cluster test result report
proc write_cluster_test_result_report {descr} {
    # regsub testreport $::cfgfile cluster_testreport clustercfg
