...
start_server {tags {"zset"}} {
    ...
    proc basics {encoding} {
        ...
        test "Check encoding - ziplist" {
            r del ztmp
            r zadd ztmp 10 x
            assert_encoding ziplist ztmp
        }

        test "ZSET basic ZADD and score update - ziplist" {
            r del ztmp
            r zadd ztmp 10 x
            r zadd ztmp 20 y
            r zadd ztmp 30 z
        ...
        test "ZSET element can't be set to NaN with ZADD - ziplist" {
        ...
        test "ZADD XX option without key - ziplist" {
            r del ztmp
            assert {[r zadd ztmp xx 10 x] == 0}
            assert {[r type ztmp] eq {none}}
        }

        test "ZADD XX existing key - ziplist" {
            r del ztmp
            r zadd ztmp 10 x
        ...
        test "ZRANK/ZREVRANK basics - ziplist" {
            r del zranktmp
        ...
        test "ZRANK - after deletion - ziplist" {
            r zrem zranktmp y
            assert_equal 0 [r zrank zranktmp x]
            assert_equal 1 [r zrank zranktmp z]
        }

        test "ZINCRBY - can create a new sorted set - ziplist" {
            r del zset
        ...
        test "ZINCRBY - increment and decrement - ziplist" {
    ...
    proc stressers {encoding} {
        ...
        test "ZSCORE - ziplist" {
            ...
            assert_encoding ziplist zscoretest
            for {set i 0} {$i < $elements} {incr i} {
                assert_equal [lindex $aux $i] [r zscore zscoretest $i]
            }
        }

        test "ZSCORE after a DEBUG RELOAD - ziplist" {
            r del zscoretest
            set aux {}
            for {set i 0} {$i < $elements} {incr i} {
                set score [expr rand()]
        ...
        test "ZSET sorting stresser - ziplist" {
            set delta 0
            for {set test 0} {$test < 2} {incr test} {
                ...
                assert_encoding ziplist myzset
        ...
        test "ZRANGEBYSCORE fuzzy test, 100 ranges in $elements element sorted set - ziplist" {
            set err {}
            r del zset
            for {set i 0} {$i < $elements} {incr i} {
        ...
        test "ZRANGEBYLEX fuzzy test, 100 ranges in $elements element sorted set - ziplist" {
            set lexset {}
            r del zset
        ...
        test "ZSETs skiplist implementation backlink consistency test - ziplist" {
            set diff 0
            for {set j 0} {$j < $elements} {incr j} {
        ...
        test "ZSETs ZRANK augmented skip list stress testing - ziplist" {
            set err {}
            r del myzset
