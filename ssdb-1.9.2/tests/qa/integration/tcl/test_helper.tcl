...
# redis instance test result report xml path
set ::cfgdir "../../../../../jenkins/report/integration/"
set curpath [pwd]
cd $::cfgdir
set ::cfgdir [pwd]
cd $curpath
set ::cfgfile [file join $cfgdir "tcl_testreport.xml"]
exec mkdir -p $::cfgdir
exec rm -f $::cfgfile
exec touch $::cfgfile
...
proc read_from_test_client fd {
    ...
    if {$status eq {ready}} {
        ...
        write_redis_test_result_report [format {</testsuite>}]
        ...
        write_redis_test_result_report "[format {<testcase name="%02d-%s" classname="%s">} \
        $::steps $testcase $classname [expr [clock seconds]-$::last_test]]"
        write_redis_test_result_report "[format {<failure type="CASE_FAILED" message="%s FAILED">%s</failure>} \
        $testcase $report]"
            write_redis_test_result_report "</testcase>"
        set ::last_test [expr [clock seconds]]
...
proc signal_idle_client fd {
    ...
    if {$::next_test != [llength $::all_tests]} {
        ...
        write_redis_test_result_report [format {<testsuite name="%s">} [lindex $::all_tests $::next_test]]
...
proc test_client_main server_port {
    ...
    write_redis_test_result_report "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>"
    write_redis_test_result_report "<testsuites>"
    while 1 {
